...
contract Reward is ERC721, Owned {
    ...
    constructor(string memory _name, string memory _symbol, address _renderer) Owned(msg.sender) {
        name = _name;
        symbol = _symbol;
    ...
    function mint(address _to, string memory _editorUsername, string memory _rewardDate, uint256 _position)
        public
        returns (bool)
    {
    ...
    function _renderSVG(Editor memory _editor) internal view returns (string memory) {
        ...
        return renderer.renderReward(
            _editor._editorAddress, _editor._editorUsername, _editor._rewardDate, _editor._position
        );
    }
}
